...
/// RANDOM FOTO BY YERIKO
                   case 'darkjokes':
				reply(mess.wait)
				 data = fs.readFileSync('./filemedia/jokes.js')
                 jsonData = JSON.parse(data)
                 randIndex = Math.floor(Math.random() * jsonData.length)
                 randKey = jsonData[randIndex]
                 darkjokes = await getBuffer(randKey.result)
                 dha.sendMessage(from, darkjokes, image, {quoted: mek, caption: '🗿🔦'})
				 break
                 case 'aesthetic': case 'anjing': case 'blackpink': case 'cecan': case 'cogan': case 'boneka': case 'hekel': case 'justina': case 'kpop': case 'kucing': case 'mobil': case 'motor': case 'profil': case 'pubg': case 'rose': case 'ryujin': case 'wallhp':
				 reply(mess.wait)
				 data = fs.readFileSync(`./filemedia/${command}.json`)
                 jsonData = JSON.parse(data)
                 randIndex = Math.floor(Math.random() * jsonData.length)
                 randKey = jsonData[randIndex]
                 randommm = await getBuffer(randKey)
                 dha.sendMessage(from, randommm, image, {quoted: mek, caption: `Foto *${command}*`})
				break                 
/*                   case 'patrik': case 'milf': case 'meme': case 'gura':
				reply(mess.wait)
				 data = fs.readFileSync(`./filemedia/${command}.js`)
                 jsonData = JSON.parse(data)
                 randIndex = Math.floor(Math.random() * jsonData.length)
                 randKey = jsonData[randIndex]
                 darkjoks = await getBuffer(randKey)
                 dha.sendMessage(from, darkjoks, image, {quoted: mek, caption: `*${command}*`})
				break        
				//random txt 
				...
				randIndex = Math.floor(Math.random() * jsonData.length)
				randKey = jsonData[randIndex]
				quotes1 = await fetchJson(randKey)
				reply(`${quotes1.quotes}`)
				break*/
